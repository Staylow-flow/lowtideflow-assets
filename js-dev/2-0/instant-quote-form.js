...
(function (global) {
  ...
  function releaseWebflowFormLoading(form) {
    ...
    function unlock() {
      ...
      var submit = document.getElementById('iq-form-submit');
      if (!submit || submit.classList.contains('is-submitting')) return;
  ...
  function validateForm() {
    ...
    if (emailClass.kind === 'invalid') {
      return 'Please enter a valid email address with a real domain (e.g. name@company.com).';
    }
    if (emailClass.kind === 'typo') {
      return (
        'Did you mean ' +
        emailClass.suggestion +
        '? Please fix the email before submitting.'
      );
    }
